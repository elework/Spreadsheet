...
public class Spreadsheet.UI.MainWindow : ApplicationWindow {
            ...
            foreach (var page in value.pages) {
                ...
                foreach (var cell in page.cells) {
                    ...
                    if (cell.selected) {
                        ...
                        style_modal.update_style_history.connect ((new_font_style, old_font_style) => {
                            update_font_style (new_font_style, old_font_style);
                        });
    ...
    public void update_font_style (Gdk.RGBA new_font_style, Gdk.RGBA old_font_style) {
        if (active_sheet.selected_cell != null) {
            HistoryManager.instance.do_action (new HistoryAction<Gdk.RGBA?, Cell> (
                ...
                (_font_style, _target) => {
                    ...
                    target.font_style.fontcolor = new_font_style;
                    stdout.printf ("Old: %s\n", old_font_style.to_string ());
                    stdout.printf ("New: %s\n", new_font_style.to_string ());

                    var undo_data = old_font_style;
                    return new StateChange<Gdk.RGBA?> (undo_data, new_font_style);
                },
                (_font_style, _target) => {
